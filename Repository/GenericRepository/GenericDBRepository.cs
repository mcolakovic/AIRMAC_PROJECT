...
namespace Repository.GenericRepository
{
    public class GenericDBRepository : IRepository<IDomainObject>
    {
        ...
        public IDomainObject Credentials(IDomainObject t)
        {
            Type type = AdminRepository.Instance.GetType();
            PropertyInfo property = type.GetProperty($"{t.TableName[t.TableNameIndex]}");
           //return ((List<IDomainObject>)property.GetValue(AdminRepository.Instance)).Contains(t);

            return ((List<IDomainObject>)property.GetValue(AdminRepository.Instance)).SingleOrDefault(p => p.Condition[0] == t.Condition[0] && p.Condition[1] == t.Condition[1]);
